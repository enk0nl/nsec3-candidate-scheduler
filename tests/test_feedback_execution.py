...
def test_runtime_reached_updates_skip_from_scaled_progress(monkeypatch, tmp_path, make_context, write_lines):
    ...
    monkeypatch.setattr('nsec3_candidate_scheduler.feedback.execution.run_cmd', lambda cmd: (4, '{"progress":[73500000,999],"recovered_salts":[0,50000]}', ''))
...
def test_feedback_execution_never_launches_with_skip_equal_total(monkeypatch, tmp_path, make_context, write_lines):
    ...
    launched = []
    monkeypatch.setattr('nsec3_candidate_scheduler.feedback.execution.run_cmd', lambda cmd: launched.append(cmd) or (0, '', ''))
    result = run_feedback_dictionary_slice(arm, ctx)
    assert launched == []
