...
def expand_feedback_bases(new_discoveries: List[str]) -> Dict[str, int]:
    ...
    for raw_discovery in new_discoveries:
        ...
        for common in COMMON_FEEDBACK_LABELS:
...
def main() -> int:
    ...
    if not any(st.name == "feedback" for st in states):
        states.append(ArmState(name="feedback", arm_type="feedback", config={"name": "feedback", "type": "feedback"}))
    for st in states:
    ...
    while True:
        ...
        feedback_stats = expand_feedback_bases([v for _, v in new_pairs])
