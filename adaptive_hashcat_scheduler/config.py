...
def _validate_amass_osint(arm: dict[str, Any]) -> dict[str, Any]:
    ...
    arm['domains_list'] = domains_list
    arm['domains_arg'] = domains_arg
    defaults = {
        'amass_binary': 'amass', 'start_on_run_start': True, 'poll_interval_seconds': 5,
        'run_immediately_when_ready': True, 'max_candidates': None, 'dedupe': True,
        'include_single_label': True, 'include_multi_label': True, 'require_min_version': True,
    ...
    if arm.get('require_min_version', True):
        try:
            proc = subprocess.run([arm['amass_binary'], '-version'], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False, timeout=10)
            ver = parse_amass_version((proc.stdout or '') + '\n' + (proc.stderr or ''))
            if ver is not None and ver < MIN_AMASS_VERSION:
                raise ValueError('amass_osint requires Amass version >= 5.1.1')
        except FileNotFoundError:
            raise ValueError(f'amass binary not found: {arm["amass_binary"]}')
