...
class DictionaryArm(Arm):
    ...
    def run_slice(self, context):
        skip=self.next_skip
        cmd=build_hashcat_command(context.hashcat_bin,context.hash_mode,0,context.slice_seconds,context.potfile,context.hashes,candidate=self.config['wordlist'],skip=skip,limit=None)
