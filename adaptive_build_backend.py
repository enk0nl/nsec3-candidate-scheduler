...
NAME = 'adaptive_hashcat_scheduler'
DIST = 'adaptive_hashcat_scheduler-0.1.0.dist-info'
PROJECT = 'adaptive-hashcat-scheduler'
VERSION = '0.1.0'


def _metadata() -> str:
    return f"Metadata-Version: 2.1\nName: {PROJECT}\nVersion: {VERSION}\nSummary: Adaptive scheduler for hashcat-based NSEC3 candidate validation experiments\nRequires-Python: >=3.10\nProvides-Extra: test\nRequires-Dist: pytest; extra == 'test'\n"


def _wheel() -> str:
    return "Wheel-Version: 1.0\nGenerator: adaptive-build-backend\nRoot-Is-Purelib: true\nTag: py3-none-any\n"
...
def _write_metadata_dir(path: Path) -> None:
    ...
    (dist / 'entry_points.txt').write_text('[console_scripts]\nadaptive-hashcat-scheduler=adaptive_hashcat_scheduler.cli:main\n', encoding='utf-8')
...
def _build(wheel_directory, editable: bool):
    wheel_name = 'adaptive_hashcat_scheduler-0.1.0-py3-none-any.whl'
    ...
    if editable:
        files['adaptive_hashcat_scheduler_editable.pth'] = (str(root) + os.linesep).encode('utf-8')
    else:
        for path in (root / 'adaptive_hashcat_scheduler').rglob('*.py'):
